hriCalc = function(object, habitat, pvals, populationDensity) {
  ...
  if (!inherits(object, "Raster")) {
    ...
    object = brick(object[, lnames], filename=rasterTmpFile(prefix=paste('brick_hc_',ieco,'.grd',sep=''))) # @javier 
# The raster:::subset construction should not be necessary, but sometimes
# the method did not dispatch correctly.
  } else object = raster:::subset(object, lnames, filename=rasterTmpFile(prefix=paste('sub_hc_',ieco,'.grd',sep=''))) # @javier 
  inHA = try(mask(object, habitat, filename=rasterTmpFile(prefix=paste('m_hc_',ieco,'.grd',sep='')))) # @javier 
  if (is(inHA, "try-error")) {
    inHA= NULL
    outHA = object
  } else outHA = mask(object, habitat, inverse = TRUE, filename=rasterTmpFile(prefix=paste('m2_hc_',ieco,'.grd',sep=''))) # @javier 
# Necessary until update of raster package
  if (!is.null(inHA) && !is(inHA, "RasterBrick")) inHA = brick(inHA, filename=rasterTmpFile(prefix=paste('brick2_hc_',ieco,'.grd',sep=''))) # @javier 
  if (!is(outHA, "RasterBrick")) outHA = brick(outHA, filename=rasterTmpFile(prefix=paste('brick3_hc_',ieco,'.grd',sep=''))) # @javier 
