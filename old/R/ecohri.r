...
for (iie in istart:length(ecoregs)) {
  ...
  ecor = ecoregions[ecoregions@data[,ecoID] == ieco,]
  if (length(ecor@polygons[[1]]@Polygons) > 100 | length(ecor@polygons) > 100) ecor = gConvexHull(ecor)
  ecor = createSPComment(ecor)
  ecorr = gBuffer(ecor, width = ecoBuffer)
  if (is.null(intersect(extent(indicators), extent(ecorr)))) next
  execor = exlim(extent(ecorr),extent(indicators))
  s1 = round(system.time(indicators2 <- crop(indicators, execor))[[1]],2)
  s2 = round(system.time(indicators2 <- mask(indicators2, ecorr))[[1]],2)
  gc()
  cpm = canProcessInMemory(indicators2, n = 15)
  s22 = proc.time()
  ...
  for (ipai in 1:pai) {
    ...
    if (!cpm) {
      ...
      for (i in 1:length(paids)) {
        ...
        tmpfiles[[i]] = writeStart(tmpfiles[[i]], filename = rasterTmpFile())
        ...
        if (!is.null(minVar)) {
          ...
          tmpfiles2[[i]] = writeStart(tmpfiles2[[i]], filename = rasterTmpFile())
      ...
      tr <- blockSize(indicators2, chunksize = 5e8)
    } else {
      tr = list(n = 1, row = 1, nrows = dim(indicators2)[1])
    }
...
#          
          ...
          if (is(hriLoc, "try-error")) {
            ...
            if (!is.null(minVar)) {
              ...
              hmc$covPoly = pCov
              hmc$nonNum = NULL
              s4 = round(system.time(hriLoc2 <- hri(indispdf,  
            ...
            if (cpm) {
              ...
              rloc1 = raster(pHab1)
              tmpfiles[[jpark]] = writeValues(tmpfiles[[jpark]], getValues(rloc1),tr$row[it])
              if (!is.null(minVar)) {
                rloc2 = raster(pHab2)
                tmpfiles2[[jpark]] = writeValues(tmpfiles2[[jpark]], getValues(rloc2),tr$row[it])
              }
              s5 = 0
            }
            t4 = proc.time()[[1]]
    ...
    if (!cpm) {
      for (jpark in 1:length(paids)) {
        ...
        if (!paid %in% paErr) {
          ...
          writeRaster(tmpfiles[[jpark]], filename = paste(tiffdir, paid, "_", ieco, ".tif", sep = ""),
...
res = list(hriRes = hriRes,  hriInRes = hriInRes, errors = errors)
if (!is.null(minVar)) {
  res$hriInRes2 = hriInRes2
  res$hriRes2 = hriRes2
}
if (exists("trr")) attr(res, "tr") = trr
res
}
