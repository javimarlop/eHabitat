...
recenterPoints = function(spdf, action = NA) {
  bb = spdf
  if (bbox(bb)[1,2] > 180 | action == "sub") {
    bb@coords[bb@coords[,1] > 180,1] = bb@coords[bb@coords[,1] > 180,1] - 360
    bb@bbox = tbox = matrix(c(min(bb@coords[,1]), min(bb@coords[,2]),
          max(bb@coords[,1]), max(bb@coords[,2])), ncol = 2, 
          dimnames = list(c("x", "y"), c("min", "max")))
  } else if (bbox(bb)[1,1] < 0 | action == "add") {
    bb@coords[bb@coords[,1] < 0,1] = bb@coords[bb@coords[,1] < 0,1] + 360
    bb@bbox = tbox = matrix(c(min(bb@coords[,1]), min(bb@coords[,2]),
          max(bb@coords[,1]), max(bb@coords[,2])), ncol = 2, 
          dimnames = list(c("x", "y"), c("min", "max")))
  }  
...
size = function(envir = .GlobalEnv){
lss = sapply(ls(envir = envir),FUN = function(x) object.size(get(x)))
sort(lss/1000)
}
...
makePolygon = function(xcor = c(0,1), ycor = c(0,1), coor = NULL, pstring = NA, id = "1", 
 ...
 if (!is.null(resol)) {
   xcor = c(xcor-resol/2, xcor+resol/2)
   ycor = c(ycor-resol/2, ycor+resol/2)
 }
 mm = matrix(c(xcor[1], xcor[2], xcor[2], xcor[1], xcor[1],
               ycor[1], ycor[1], ycor[2], ycor[2], ycor[1]), ncol = 2)
 srl = list(Polygon(mm))
 Srl = list(Polygons(srl, ID = id))
 SpP = SpatialPolygons(Srl)
 if (!is.null(data)) SpP = SpatialPolygonsDataFrame(SpP, data = as.data.frame(data))
 proj4string(SpP) = CRS(pstring)
 SpP
}



plotpa = function(x,pa,z = names(x)) {
#if (is(y,"character"))
p1 = spplot(x, z, col.regions = bpy.colors(), 
    panel = function(x,y, ...){
    panel.polygonsplot(x,y, ...) 
    sp.polygons(pa, col = "red", lwd = 1)})
print(p1)
}


inCont3 = function(polys, cont) {
  coors = t(getcor(polys))
  ret = NULL
  for (i in 1:dim(coors)[1]) {
     point = coors[i,, drop = FALSE]
     ret = c(ret,as.character(over(SpatialPoints(point, proj4string = CRS(proj4string(cont))), cont)$CONTINENT))
  }
  ret = unique(ret)
  ret = ret[!is.na(ret)]
  if (length(ret) == 0) {                                                                           
    ret = NA
  } else if (length(ret) > 1) ret = "multi"
  ret
}


getcor = function(xx) sapply(xx@Polygons, FUN = function(x) x@labpt)


pInside = function(bpa, bb) {
  xorder = c(1,2,2,1)
  yorder = c(2,2,1,1)
  ret = FALSE
  for (i in 1:4) {
    point = c(bpa[1, xorder[i]],bpa[2, yorder[i]])
    if (point[1] < bb[1,2] & point[1] > bb[1,1] &  
        point[2] < bb[2,2] & point[2] > bb[2,1]) ret = TRUE
  }
  ret 
}
...
sum(unlist(lapply(poly@polygons, getAreaPolygons)))
}
    ...
    lty.hide = NULL, lty.hplot = par("lty"), log = "", ...) 
{
   ...
   if (type == "p" || type == "h") {
        ...
        if (highlight.3d && !(all(diff(daty) == 0))) {            
              at = seq(y.range[1], y.range[2], diff(y.range)/(n-1))
              datint = findInterval(daty, at, all.inside = TRUE) 
              cols = bpy.colors(n)[datint]           
              dat$col <- cols
        }
    }
    p.lab <- par("lab")
