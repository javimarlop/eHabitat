hriCalc = function(object, habitat, pvals, populationDensity) {
  ...
  if (!inherits(object, "Raster")) {
    ...
    object = brick(object[, lnames]) 
# The raster:::subset construction should not be necessary, but sometimes
# the method did not dispatch correctly.
  } else object = raster:::subset(object, lnames) 
  inHA = try(mask(object, habitat))
  if (is(inHA, "try-error")) {
    inHA= NULL
    outHA = object
  } else outHA = mask(object, habitat, inverse = TRUE)
# Necessary until update of raster package
  if (!is.null(inHA) && !is(inHA, "RasterBrick")) inHA = brick(inHA)
  if (!is(outHA, "RasterBrick")) outHA = brick(outHA)
