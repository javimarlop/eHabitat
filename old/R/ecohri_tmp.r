...
for (iie in istart:length(ecoregs)) {
  ...
  print(fnn)
  print(GDALinfo(fnn))
  s1 = round(system.time(indicators2 <<- crop(indicators, execor, filename=rasterTmpFile(prefix=paste('crop_',ieco,'.grd',sep=''))))[[1]],2) # creation of target raster? @javier
  ...
  s2 = round(system.time(indicators2 <<- mask(indicators2, ecorr, filename=rasterTmpFile(prefix=paste('mask_',ieco,'.grd',sep=''))))[[1]],2) # MASK, 
  ...
  for (ipai in 1:pai) { # starts looping over subsets of parks up to line 330
    ...
    if (!cpm) { # if cpm is FALSE, so the processing cannot take place in the memory @javier
      ...
      for (i in 1:length(paids)) {
        ...
        tmpfiles[[i]] = writeStart(tmpfiles[[i]], filename = rasterTmpFile(prefix=paste('r_',ieco,'_',sep=''))) # @javier
        ...
        if (!is.null(minVar)) {
          ...
          tmpfiles2[[i]] = writeStart(tmpfiles2[[i]], filename = rasterTmpFile(prefix=paste('r_',ieco,'_',sep=''))) # @javier
