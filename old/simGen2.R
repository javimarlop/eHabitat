...
gIntersects(ecoregions,parks,byid=T)->ep # @javier
base<-dim(ep)[2]
...
ecoregs = ecoregs_tmp2[22:50,] # :825 changed by @javier 13.08.13 in order to continue from where it crashed
...
# all nodes have returned.
  s3 = system.time(results <- clusterApplyLB(cl, ecoregs$ecoregs, fun = mecohri2, # call to mecohri2 @javier
